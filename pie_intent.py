...
class PIEIntent(object):
    ...
    def load_images_and_process(self,
        ...
        for seq, pid in zip(img_sequences, ped_ids):
            ...
            for imp, b, p in zip(seq, bbox_sequences[i], pid):
                ...
                if os.path.exists(img_save_path) and not regen_pkl:
                    ...
                    if context:
                        ...
                        bbox = jitter_bbox(imp, [b],'same', 2)[0]
            ...
            # scene
            scene_save_path=os.path.join(img_save_folder, img_name+'_scene'+p[0]+'.pkl')
            if os.path.exists(scene_save_path) and not regen_pkl:
                with open(scene_save_path, 'rb') as fid:
                    try:
                        scene = pickle.load(fid)
                    except:
                        scene = pickle.load(fid, encoding='bytes')
            else:
                scene_data=load_img(imp)
                scene_array=img_to_array(scene_data)
                preprocessed_scene = vgg16.preprocess_input(scene_array)
                expanded_scene = np.expand_dims(preprocessed_scene, axis=0)
                scene= context_model.predict(expanded_scene)
                if not os.path.exists(img_save_folder):
                    os.makedirs(img_save_folder)
                with open(scene_save_path, 'wb') as fid:
                    pickle.dump(scene, fid, pickle.HIGHEST_PROTOCOL)
            scenes.append(scene)
    ...
    def get_test_data(self, data, train_params, seq_length, overlap, location, context, lstm):
        """
        ...
        test_img,test_scene = self.load_images_and_process(images,
                                                bboxes,
                                                ped_ids,
                                                context=context,
                                                lstm=lstm,
        ...
        output = output[:, 0]
        if location:
            test_data = ([test_img, decoder_input], output)
        else:
            test_data = (test_img, output)
        return test_data
    ...
    def pie_convlstm_encdec(self, location):
        '''
        ...
        scene=scene_input=Input(shape=(1,33,60,512), name="scene_input")
        attention_output=attention(scene,input_data)
        interm_input=Concatenate(axis=1)([encoder_input,attention_output])
        ...
        self.train_model = Model(inputs=[encoder_input, scene_input],
    ...
    def train(self,
        ...
        train_img,train_scene = self.load_images_and_process(train_d['images'],
                                                  train_d['bboxes'],
                                                  train_d['ped_ids'],
                                                  context=context,
                                                  lstm=lstm,
                                                  ...
                                                  save_path=self.get_path(type_save='data',
                                                                          ...
                                                                          model_name='vgg16_'+'none', #"dense",
                                                                          data_subset = 'train'))
        val_img,val_scene = self.load_images_and_process(val_d['images'],
                                                val_d['bboxes'],
                                                train_d['ped_ids'],
                                                context=context,
                                                lstm=lstm,
        ...
        # if lstm:
        train_model = self.pie_convlstm_encdec(location)
        # else:
        # train_model = densenet_model()
        # train_model.summary()
        ...
        if location:
            train_data = ([train_img, train_d['decoder_input'], train_scene], train_d['output'])
            val_data = ([val_img, val_d['decoder_input'], val_scene], val_d['output'])
        else:
            train_data = ([train_img, train_scene], train_d['output'])
            val_data = ([val_img, val_scene],val_d['output'])
            
        #attention("E:\\PIEPredict-master\\PIE_dataset\\images\\set01\\video_0001\\01013.png",train_img)
